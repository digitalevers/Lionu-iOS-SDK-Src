...
//  VideoEditorDemo
//
//  Created by MacBook on 2020/11/7.
...
//    [CHDeviceTool getWifiInfo];
    ...
    //获取WIFI的MAC地址
    NSString * MACAdress = [DeviceTool getWifiMACAdress];
    if (MACAdress) {
        [deviceParams setObject:MACAdress forKey:@"macAdress"];
    }
    
    //获取IP地址
    NSString * IPAddress = [DeviceTool getIPAddress];
    [deviceParams setObject:IPAddress forKey:@"ipAddress"];
    
    //APP名字
    NSString * APPName = [DeviceTool getAPPName];
    if (APPName) {
    ...
    //APP Build版本
    NSString * APPBuild = [DeviceTool getAPPBuild];
...
+ (NSString *)getUUID{
    ...
    if ([LionuDeviceInfoSDK shareInstance].configModel.isMD5) {
        //对uuid进行md5
        UUIDString = [DeviceTool md5HashOfString:UUIDString];
    }
    return UUIDString;
}
...
+(NSString *)getIDFA{
    ...
    if ([LionuDeviceInfoSDK shareInstance].configModel.isMD5) {
        //对uuid进行md5
        idfa = [DeviceTool md5HashOfString:idfa];
    }
    return idfa;
}
...
+(NSString *)getWifiName{
    return (NSString *)[DeviceTool getWifiInfo][@"SSID"];
}

/**获取WIFI的MAC地址*/
+(NSString *)getWifiMACAdress{
    return (NSString *)[DeviceTool getWifiInfo][@"BSSID"];
}
